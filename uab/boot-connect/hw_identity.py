...
# BIOS·제품 스펙(16GB·512GB)과 설치 UI 표기 일치 — OS 가용량·GiB→GB 보정
_RAM_SPEC_GB = (128, 64, 32, 16, 8, 4)
_SSD_SPEC_GB = (2048, 1024, 512, 256, 128)


def normalize_ram_gb_from_memtotal_kb(kb: int) -> int:
    """MemTotal(GiB) → 장착 RAM 스펙. iGPU/UMA 예약(~1GB)으로 16GB→15GiB 보이는 경우 16으로."""
    gib = kb / 1024 / 1024
    for spec in _RAM_SPEC_GB:
        if gib >= spec * 0.875:
            return spec
    return max(1, round(gib))


def normalize_ssd_gb_from_bytes(size_bytes: int) -> int:
    """디스크 바이트 → 마케팅 GB(10⁹). BIOS 512GB ≈ lsblk 477GiB."""
    if size_bytes <= 0:
        return 0
    decimal_gb = size_bytes / 1_000_000_000
    rounded = max(1, round(decimal_gb))
    for spec in _SSD_SPEC_GB:
        if abs(rounded - spec) <= 8:
            return spec
    return rounded
        ...
        with open("/proc/meminfo", encoding="utf-8") as f:
            for line in f:
                if line.startswith("MemTotal:"):
                    kb = int(line.split()[1])
                    return normalize_ram_gb_from_memtotal_kb(kb)
    except OSError:
        pass
    return 0
        ...
        for line in out.splitlines()[1:]:
            ...
            if len(parts) >= 3 and parts[1] == "disk" and parts[2] == "0":
                ...
                if nbytes > best_bytes:
                    best_bytes = nbytes
        return normalize_ssd_gb_from_bytes(best_bytes) if best_bytes > 0 else 0
    except (OSError, subprocess.SubprocessError, ValueError):
        return 0


def _temp_c_max() -> float | None:
    temps: list[float] = []
    try:
...
def collect_hw_runtime_specs() -> dict:
    """HW 단계 표시용 — CPU · RAM · SSD · 온도 (교체 무관 런타임 값)."""
    cpu = _cpu_model()
    ram = _ram_gb()
    ssd = _ssd_gb()
    temp = _temp_c_max()
    return {
        "cpu_model": cpu or None,
        "ram_gb": ram or None,
        "ssd_gb": ssd or None,
        "temp_c": temp,
    }
