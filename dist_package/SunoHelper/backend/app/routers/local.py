...
async def api_local_status() -> dict[str, Any]:
    # license 상태 조회가 어떤 이유로 실패해도 이 엔드포인트는 반드시 200으로
    # 응답해야 한다 — whick.org 웹 대시보드의 설치 감지가 여기에 의존한다.
    try:
        lic = license_service.license_status()
    except Exception:
        lic = {}
    # API키 보관 여부 (2026-09-08 API키 전환) — license.json의 api_key 존재만 확인(원문 노출 없음)
    has_api_key = False
    try:
        has_api_key = bool(license_service._load_saved().get("api_key"))
    except Exception:
        has_api_key = False
    return {
        "installed": True,  # 이 API에 접근 가능 == 설치됨
        "version": APP_VERSION,
        "web_launch": True,  # 웹 설치 흐름 상수 허용 (lock 유무와 무관 — 대시보드 버튼 전환용)
