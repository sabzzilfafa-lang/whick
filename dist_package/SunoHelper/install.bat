...
set PYTHONUTF8=1
python -m pip install -r backend\requirements.txt -q --no-input --disable-pip-version-check
if errorlevel 1 (
...
echo [4/4] Enabling web launch + starting backend...
copy /y nul data\web_launch.lock >nul

rem --- stop stale backend on port 8765 (old version blocks bind), then start fresh ---
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\kill_ports.ps1" >nul 2>&1
ping 127.0.0.1 -n 3 >nul
set "PYW=%CD%\backend\.venv\Scripts\pythonw.exe"
if not exist "%PYW%" set "PYW=%CD%\backend\.venv\Scripts\python.exe"
start "" /b "%PYW%" "%CD%\backend\run_server.py"
