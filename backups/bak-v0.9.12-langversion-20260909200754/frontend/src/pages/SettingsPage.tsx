  ...
  return (
    <div className="model-select-wrap">
      <select
        ...
        title={value}
      >
        {loading && <option value="">모델 불러오는 중...</option>}
        {!loading &&
          models.map((m) => (
      ...
      {!loading && providerId === "openrouter" && models.length > 7 && (
        <span className="model-hint">OpenRouter 전체 모델 목록</span>
      )}
    </div>
  );
}
...
export default function SettingsPage() {
  ...
  const handleWhickKeyActivate = async () => {
    ...
    if (!key) {
      setError("whick.org 내 계정에서 발급한 API 키를 입력하세요");
      return;
    }
    ...
    try {
      ...
      setHasApiKey(true);
      setWhickKeyInput("");
      setMessage("API 키가 확인되어 활성화되었습니다");
    } catch (e) {
      setError(e instanceof Error ? e.message : "API 키 활성화에 실패했습니다");
    } finally {
  ...
  const handleTest = async (providerId: string, keyField: string) => {
    ...
    try {
      ...
      setMessage(
        `${providers.find((p) => p.id === providerId)?.name} 연결 성공 (${result.model_count}개 모델)`
      );
  ...
  const applyRecommendedModels = () => {
    ...
    setMessage("가성비 권장 모델·창의성이 선택되었습니다. 「설정 저장」을 눌러 적용하세요.");
    setError("");
  };

  const handleYoutubeSave = async () => {
    if (!ytClientId.trim()) {
      setError("OAuth 클라이언트 ID를 입력하세요.");
      return;
    }
    if (!ytClientSecret.trim() && !youtube?.client_secret_set) {
      setError("OAuth 클라이언트 보안 비밀을 입력하세요.");
      return;
    }
    setYtSaving(true);
    setError("");
    ...
    try {
      ...
      setYoutube(updated);
      setYtClientSecret("");
      setMessage("YouTube OAuth 설정이 저장되었습니다.");
    } catch (e) {
  ...
  const formatYoutubeError = (e: unknown) => {
    ...
    if (msg.includes("요청 실패") || msg.includes("Not Found")) {
      return "YouTube API에 연결할 수 없습니다. stop.bat 실행 후 start.bat으로 백엔드를 다시 시작하세요.";
    }
    return msg;
  };
  ...
  const handleYoutubeConnect = async () => {
    if (!ytClientId.trim() && !youtube?.client_id) {
      setError("먼저 OAuth 클라이언트 ID와 보안 비밀을 입력하고 저장하세요.");
      return;
    }
    ...
    try {
      ...
      if (!popup) {
        setMessage("팝업이 차단되었습니다. 같은 탭에서 Google 로그인 페이지로 이동합니다.");
        window.location.href = auth_url;
        return;
      }
      setMessage("Google 로그인 창에서 채널을 승인해 주세요.");
      const poll = setInterval(async () => {
        try {
          ...
          if (st.connected) {
            ...
            setMessage(`${st.channel_title || "채널"} 연결 완료`);
            popup.close();
          }
        } catch {
          /* ignore */
        ...
        if (popup.closed) {
          ...
          try {
            ...
            if (st.connected) {
              ...
              setMessage("로그인 창이 닫혔습니다. 연결이 완료되지 않았으면 다시 시도하세요.");
            }
          } catch (e) {
            setError(formatYoutubeError(e));
          }
  ...
  const handleImportBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMessage(result.message || "백업 복원 완료");
    } catch {
      setError("백업 복원 실패");
    }
  };
  ...
  if (!settings) {
    return (
      <div>
        <div className="error">{error || "설정을 불러올 수 없습니다. 백엔드가 실행 중인지 확인하세요."}</div>
      </div>
    );
  }
  ...
  return (
    <div>
          ...
          <div className="card" style={{ marginBottom: "1.5rem" }}>
            ...
            {licenseState === "valid" || licenseState === "grace" ? (
              <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>
                활성화됨{licenseEmail ? ` — ${licenseEmail}` : ""}
                {licenseState === "grace" ? " (갱신 만료 임박 — 다음 작업 시 자동 갱신)" : ""}
              </p>
            ) : (
              <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>
                whick.org에 회원가입 후 <b>내 계정 → API 키</b>에서 발급한 키를 입력하세요.
                키가 있어야 작업(파이프라인)을 실행할 수 있습니다.
              </p>
            )}
            <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap" }}>
              <input
                ...
                placeholder={hasApiKey ? "새 API 키로 교체 (whick_…)" : "whick_… 로 시작하는 API 키"}
              ...
              >
                {whickKeyBusy ? "확인 중…" : hasApiKey ? "키 교체" : "활성화"}
              </button>
              <a
            ...
            <div key={p.id} className="provider-card">
              ...
              {isKeySet(p.key_field) && (
                <p style={{ fontSize: "0.8rem", marginTop: "0.5rem" }}>
                  등록됨: <code>{getMasked(p.key_field)}</code>
                </p>
              )}
              <div className="form-group" style={{ marginTop: "0.75rem", marginBottom: "0.5rem" }}>
                <input
                  ...
                  placeholder={isKeySet(p.key_field) ? "변경 시에만 입력" : "API 키 입력"}
                />
              </div>
              <button
      ...
      <div className="card" style={{ marginBottom: "1.5rem" }}>
        <div className="card-title">작업별 AI 설정</div>
        <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>
      ...
      {tab === "youtube" && (
        ...
        {youtube ? (
          <div style={{ fontSize: "0.8rem", marginBottom: "1rem" }}>
            <p style={{ marginBottom: "0.35rem" }}>현재 앱이 사용하는 URI:</p>
            <code style={{ display: "block", marginBottom: "0.5rem" }}>{youtube.redirect_uri}</code>
            <p className="meta">Google에 둘 다 등록해 두면 편합니다:</p>
        ...
        <div className="form-group">
          <label>리디렉션 호스트 (Google에 등록한 주소와 동일하게)</label>
          <select
        ...
        <div className="form-group">
          <label>OAuth 클라이언트 보안 비밀</label>
          <input
            type="password"
            value={ytClientSecret}
            onChange={(e) => setYtClientSecret(e.target.value)}
            placeholder={youtube?.client_secret_set ? "변경 시에만 입력" : "클라이언트 보안 비밀"}
          />
          {youtube?.client_secret_set && (
            <span className="meta">등록됨: {formatMaskedSecret(youtube.client_secret_masked)}</span>
          )}
        </div>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
          <button className="btn btn-secondary btn-sm" onClick={handleYoutubeSave} disabled={ytSaving}>
            {ytSaving ? "저장 중..." : "OAuth 저장"}
          </button>
          <button
          ...
          >
            {ytConnecting ? "연결 대기 중..." : "YouTube 채널 연결"}
          </button>
          {youtube?.connected && (
            <>
              <span className="badge badge-ok">연결됨: {youtube.channel_title}</span>
